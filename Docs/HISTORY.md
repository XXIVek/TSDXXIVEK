...
## 2026-09-15 — ИСПРАВЛЕНИЕ: Отображение лога ТСД в LogTextFragment

### Проблема

При нажатии кнопки "Открыть лог" в `SetgeneralFragment` экран лога был пустым, хотя файл `/Download/logTSD.dat` содержал данные.

### Причина

`LogTextFragment.onResume()` читал лог из переменной `msg_log`, которая хранится **только в памяти** и сбрасывается при перезапуске приложения. Фактический лог-файл не читался.

### Решение

| Файл | Изменение |
|------|-----------|
| `LogTextFragment.kt` | Добавлен метод `readLogFile()` для чтения файла `/storage/emulated/0/Download/logTSD.dat`. В `onResume()` теперь читается файл, а не переменная `msg_log` |
| `LogUtil.kt` | Исправлен путь к файлу с `"sdcard/Download/logTSD.dat"` (относительный) на `"/storage/emulated/0/Download/logTSD.dat"` (абсолютный) |
